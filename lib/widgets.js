(function() {
  ...
  Date.findFirstWeekFirstDay = function(year) {
    var d, day;
  ...
  Date.prototype.getWeek = function(dowOffset) {
    ...
    start = Date.findFirstWeekFirstDay(this.getFullYear());
    dif = this - start - this.getTimezoneOffset() * MILLISECONDS_IN_MINUTE;
  ...
  Calendar = (function(_super) {
    ...
    Calendar.prototype.toggleState = function(td, date) {
      var sameDate, sameMonth, sameWeek, value;
      value = this.get("value");
      sameDate = date.getDate() === value.getDate();
      sameMonth = date.getMonth() === value.getMonth();
      sameWeek = date.getWeek() === value.getWeek();
      switch (this.get("mode")) {
        case "date":
          if (sameDate && sameMonth) td.addClass("selected");
          break;
        case "month":
          if (sameMonth) td.addClass("selected");
          break;
        case "week":
          if (sameWeek) td.addClass("selected");
      }
      if (!sameMonth) td.addClass("blurred");
  ...
  weekToString = function(date) {
    return "" + (fill(date.getFullYear(), 4)) + "-W" + (fill(date.getWeek()));
  };
