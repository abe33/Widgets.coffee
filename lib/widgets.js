(function() {
  var AbstractDateInputWidget, AbstractMode, BGRMode, BUILDS, BuildUnit, Button, Calendar, CheckBox, ColorInput, ColorPicker, Container, DAYS, DateInput, DateTimeInput, DateTimeLocalInput, FilePicker, FocusProvidedByChild, GRBMode, HSVMode, HasChild, KeyStroke, MILLISECONDS_IN_DAY, MILLISECONDS_IN_HOUR, MILLISECONDS_IN_MINUTE, MILLISECONDS_IN_SECOND, MILLISECONDS_IN_WEEK, MenuList, MenuModel, Module, MonthInput, NumericWidget, RGBMode, Radio, RadioGroup, SHVMode, SingleSelect, Slider, SquarePicker, Stepper, TableBuilder, TextArea, TextInput, TimeInput, VHSMode, ValueInRange, WeekInput, Widget, WidgetPlugin, colorObjectFromValue, colorObjectToValue, dateFromString, dateToString, datetimeFromString, datetimeLocalFromString, datetimeLocalToString, datetimeToString, fill, findFirstWeekFirstDay, getWeekDate, hex2rgb, hsv2rgb, i, isValidChannel, isValidColor, isValidDate, isValidDateTime, isValidDateTimeLocal, isValidHSV, isValidHue, isValidMonth, isValidNumber, isValidPercentage, isValidRGB, isValidTime, isValidValue, isValidWeek, j, keys, keystroke, monthFromString, monthToString, radioProcessor, rgb2hex, rgb2hsv, safeInt, timeFromString, timeToString, validationRegexp, weekFromString, weekToString,
  ...
  Widget = (function(_super) {
    ...
    Widget.prototype.toString = function() {
      var details;
      details = this.get("id") ? "(id=\"" + (this.get("id")) + "\")" : "";
      return "[object " + this.constructor.name + details + "]";
    };
  ...
  NumericWidget = (function(_super) {
    ...
    NumericWidget.mixins(ValueInRange);
  ...
  Slider = (function(_super) {
    ...
    function Slider(target) {
      ...
      if (this.get("step") == null) this.properties.step = 1;
      if (this.get("min") == null) this.properties.min = 0;
      if (this.get("max") == null) this.properties.max = 100;
  ...
  SingleSelect = (function(_super) {
    ...
    function SingleSelect(target) {
      ...
      if (this.hasTarget) {
        ...
        if (!isNaN(size)) this.properties.size = size;
  ...
  Calendar = (function(_super) {
    ...
    function Calendar(value) {
      ...
      this.createProperty("mode", "date");
      this.createProperty("value", value != null ? value : new Date);
      this.updateDummy();
    }

    Calendar.prototype.createDummy = function() {
      var dummy;
      dummy = $("<span class='calendar'>                     <table></table>                   </span>");
      return dummy;
    };

    Calendar.prototype.updateDummy = function() {
      var date, monthStartDay, value,
        _this = this;
      value = this.get("value").clone();
      this.updateCells(value);
      value.setDate(1);
      monthStartDay = value.getDay();
      date = value.clone();
      date.incrementDate(-monthStartDay);
      return this.dummy.find("td").each(function(i, o) {
        var td;
        td = $(o);
    ...
    Calendar.prototype.updateCells = function(value) {
      var day, header, line, table, x, y, _i, _len, _ref, _results;
      ...
      table.append(header);
      _results = [];
      for (y = 0, _ref = this.linesNeeded(value); 0 <= _ref ? y <= _ref : y >= _ref; 0 <= _ref ? y++ : y--) {
        line = $("<tr></tr>");
        for (x = 0; x <= 6; x++) {
          line.append("<td></td>");
        }
  ...
  AbstractDateInputWidget = (function(_super) {
    ...
    AbstractDateInputWidget.mixins(ValueInRange);
    ...
    function AbstractDateInputWidget(target, defaultStep) {
      ...
      this.properties.step = isNaN(step) ? defaultStep : step;
      if (min != null) {
        minDate = this.snapToStep(this.valueToDate(min));
        this.properties.min = this.dateToValue(minDate);
      }
      if (max != null) {
        maxDate = this.snapToStep(this.valueToDate(max));
        this.properties.max = this.dateToValue(maxDate);
      }
      this.properties.date = this.fitToRange(date, minDate, maxDate);
      this.properties.value = this.dateToValue(date);
    ...
    AbstractDateInputWidget.prototype.snapToStep = function(value) {
      ...
      ms = value.getTime();
      step = this.get("step");
      if (step != null) value.setTime(ms - (ms % (step * MILLISECONDS_IN_SECOND)));
      return value;
    };
    ...
    AbstractDateInputWidget.prototype.set_date = function(property, value) {
      ...
      this.properties[property] = this.fitToRange(value, min, max);
    ...
    AbstractDateInputWidget.prototype.set_min = function(property, value) {
      ...
      this.properties[property] = this.dateToValue(this.snapToStep(this.valueToDate(value)));
      this.valueToAttribute(property, value);
  ...
  TimeInput = (function(_super) {
    ...
    TimeInput.prototype.mousemove = function(e) {
      ...
      if (this.dragging) {
        ...
        step = this.get("step");
        this.set("date", new Date(ms + dif * step * MILLISECONDS_IN_SECOND));
        return this.pressedY = y;
      }
    };
  ...
  dateToString = function(date) {
    return ["" + (fill(date.getFullYear(), 4)), "" + (fill(date.getMonth() + 1)), "" + (fill(date.getDate()))].join("-");
  };
  ...
  DateInput = (function(_super) {
    ...
    DateInput.prototype.mouseup = function(e) {
  ...
  datetimeToString = function(date) {
    ...
    return ["" + (dateToString(date)) + "T", "" + (timeToString(date)), "" + sign + (fill(hours)) + ":" + (fill(minutes))].join("");
  };
  ...
  datetimeLocalFromString = function(string) {
    ...
    _ref = validationRegexp().exec(string), match = _ref[0], year = _ref[1], month = _ref[2], day = _ref[3], hours = _ref[4], minutes = _ref[5], seconds = _ref[6], milliseconds = _ref[7];
    pI = parseInt;
  ...
  this.monthFromString = monthFromString;

  this.isValidWeek = isValidWeek;

  this.weekToString = weekToString;

  this.weekFromString = weekFromString;
  ...
  this.isValidDateTimeLocal = isValidDateTimeLocal;

  this.datetimeLocalToString = datetimeLocalToString;

  this.datetimeLocalFromString = datetimeLocalFromString;
  ...
  TableBuilder = (function() {
    ...
    TableBuilder.prototype.build = function() {
      var col, cols, colspan, hasColumnHeaders, hasRowHeaders, k, opts, row, rows, table, td, th, tr, unit, v, widget, _base, _i, _j, _k, _l, _len, _len2, _len3, _m, _ref, _ref2, _ref3, _results, _results2;
      cols = (function() {
        _results = [];
      ...
      if (hasColumnHeaders) {
        ...
        for (_k = 0, _len = cols.length; _k < _len; _k++) {
          ...
          opts.col = col;
          opts.th = th;
          th.text(this.options.columnHeaders(opts));
          tr.append(th);
        }
        table.append(tr);
      }
      for (_l = 0, _len2 = rows.length; _l < _len2; _l++) {
        ...
        if (hasRowHeaders) {
          ...
          opts.row = row;
          opts.th = th;
          th.text(this.options.rowHeaders(opts));
          tr.append(th);
        }
        for (_m = 0, _len3 = cols.length; _m < _len3; _m++) {
          ...
          if (this.options.cls != null) {
            unit = new BuildUnit(this.options.cls, this.options.args);
            widget = unit.build();
            widget.attach(td);
            opts.widget = widget;
          }
  ...
  BuildUnit = (function() {
    ...
    function BuildUnit(klass, args, set, callback) {
      this.klass = klass;
    ...
    BuildUnit.prototype.build = function() {
      var k, o;
      o = this.construct(this.klass, this.args);
      ...
      if (typeof this.callback === "function") this.callback(o);
      return o;
    };
