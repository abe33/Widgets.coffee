(function() {
  var AbstractDateInputWidget, AbstractMode, BGRMode, Button, CheckBox, ColorInput, ColorPicker, Container, DateInput, DateTimeInput, FilePicker, GRBMode, HSVMode, KeyStroke, MILLISECONDS_IN_DAY, MILLISECONDS_IN_HOUR, MILLISECONDS_IN_MINUTE, MILLISECONDS_IN_SECOND, MILLISECONDS_IN_WEEK, MenuList, MenuModel, MockWidget, MonthInput, NumericWidget, RGBMode, Radio, RadioGroup, SHVMode, SingleSelect, Slider, SquarePicker, Stepper, TextArea, TextInput, TimeInput, VHSMode, WeekInput, Widget, WidgetPlugin, area1, area2, area3, button1, button2, button3, checkbox1, checkbox2, checkbox3, colorObjectFromValue, colorObjectToValue, dateFromString, dateToString, datetimeFromString, datetimeToString, dialog, fill, findFirstWeekFirstDay, getWeekDate, group, hex2rgb, hsv2rgb, input1, input2, input3, isValidChannel, isValidColor, isValidDate, isValidDateTime, isValidHSV, isValidHue, isValidMonth, isValidNumber, isValidPercentage, isValidRGB, isValidTime, isValidValue, isValidWeek, item1, item2, item3, item4, item5, item6, item7, item8, keys, keystroke, list1, list2, list3, model, monthFromString, monthToString, picker1, picker2, picker3, radio1, radio2, radio3, rgb2hex, rgb2hsv, s, safeInt, select1, select2, select3, sinput1, sinput2, sinput3, sinput4, slider1, slider2, slider3, stepper1, stepper2, stepper3, target, testGenericDateWidgetBehaviors, timeFromString, timeToString, weekFromString, weekToString,
  ...
  Widget = (function() {
    ...
    function Widget(target) {
      ...
      this.properties = {
        ...
        required: this.booleanFromAttribute("required"),
  ...
  hsv2rgb = function(h, s, v) {
    var b, comp1, comp2, comp3, dominant, g, r, rnd;
    h = h / 60;
    s = s / 100;
    v = v / 100;
    rnd = Math.round;
  ...
  AbstractDateInputWidget = (function(_super) {
    ...
    function AbstractDateInputWidget(target) {
      var date, max, min, step, value;
      min = null;
      max = null;
      step = 0;
    ...
    AbstractDateInputWidget.prototype.checkTarget = function(target) {
      if (!this.isInputWithType(target, this.supportedType)) {
        throw "TimeInput target must be an input with a " + this.supportedType + " type";
      }
    };

    AbstractDateInputWidget.prototype.set_date = function(property, value) {
      if (!(value != null) || isNaN(value.getDate())) return this.get("date");
      if (this.dateSetProgrammatically) return value;
      value = this.fitToRange(value);
      this.set("value", this.dateToValue(value));
      return value;
    };

    AbstractDateInputWidget.prototype.set_value = function(property, value) {
      var date;
      if (!this.isValidValue(value)) return this.get("value");
      date = this.fitToRange(this.valueToDate(value));
      this.dateSetProgrammatically = true;
      this.set("date", date);
      this.dateSetProgrammatically = false;
      value = this.dateToValue(date);
      AbstractDateInputWidget.__super__.set_value.call(this, property, value);
      return value;
    };

    AbstractDateInputWidget.prototype.fitToRange = function(date) {
      var max, min;
      min = this.get("min");
      max = this.get("max");
    ...
    AbstractDateInputWidget.prototype.dateFromAttribute = function(attr, def) {
      var value;
  ...
  TimeInput = (function(_super) {
    ...
    function TimeInput(target) {
      ...
      this.valueToDate = timeFromString;
      this.dateToValue = timeToString;
      this.isValidValue = isValidTime;
      TimeInput.__super__.constructor.call(this, target);
    }
  ...
  DateInput = (function(_super) {
    ...
    function DateInput(target) {
      ...
      this.valueToDate = dateFromString;
      this.dateToValue = dateToString;
      this.isValidValue = isValidDate;
  ...
  isValidMonth = function(value) {
    if (value == null) return false;
    return /^[\d]{4}-[\d]{2}$/g.test(value);
  };

  monthFromString = function(string) {
    var d;
    d = new Date(string);
    d.setHours(0);
    return d;
  ...
  MonthInput = (function(_super) {
    ...
    function MonthInput(target) {
      ...
      this.valueToDate = monthFromString;
      this.dateToValue = monthToString;
      this.isValidValue = isValidMonth;
  ...
  isValidWeek = function(value) {
    if (value == null) return false;
    return /^[\d]{4}-W[\d]{2}$/g.test(value);
  };

  weekFromString = function(string) {
  ...
  findFirstWeekFirstDay = function(year) {
    ...
    if (day === 0) day = 7;
    if (day > 3) {
      return new Date(year, 0, 7 - day + 2, 0);
    } else {
      return new Date(year, 0, 2 - day, 0);
    }
  };

  getWeekDate = function(year, week) {
    var date, start;
  ...
  WeekInput = (function(_super) {
    ...
    function WeekInput(target) {
      ...
      this.valueToDate = weekFromString;
      this.dateToValue = weekToString;
      this.isValidValue = isValidWeek;
  ...
  datetimeFromString = function(string) {
  ...
  DateTimeInput = (function(_super) {
    ...
    function DateTimeInput(target) {
      ...
      this.valueToDate = datetimeFromString;
      this.dateToValue = datetimeToString;
      this.isValidValue = isValidDateTime;
  ...
  test("isValidTime should return true", function() {
    assertThat(isValidTime("10"));
    assertThat(isValidTime("10:10"));
    assertThat(isValidTime("10:10:15"));
    return assertThat(isValidTime("10:10:15.765"));
  });

  test("isValidTime should return false", function() {
    assertThat(!isValidTime(""));
    assertThat(!isValidTime("foo"));
    assertThat(!isValidTime("100:12:00"));
    assertThat(!isValidTime("1:0:0"));
    assertThat(!isValidTime("::"));
    return assertThat(!isValidTime(null));
  });

  test("timeToString should return valid time string", function() {
    assertThat(timeToString(new Date(1970, 0, 1, 10, 16, 52)), equalTo("10:16:52"));
    return assertThat(timeToString(new Date(1970, 0, 1, 10, 16, 52, 756)), equalTo("10:16:52.756"));
  });

  test("timeFromString should return valid dates", function() {
    ...
    return assertThat(timeFromString("10:16:52.756"), dateEquals(new Date(1970, 0, 1, 10, 16, 52, 756)));
  });

  test("time chaining conversion should always result to the same value", function() {
    return assertThat(timeFromString(timeToString(timeFromString("10:16:52"))), dateEquals(timeFromString("10:16:52")));
  });

  module("date utils tests");

  test("isValidDate should return true", function() {
    return assertThat(isValidDate("2011-12-15"));
  });

  test("isValidDate should return false", function() {
    assertThat(!isValidDate(""));
    assertThat(!isValidDate("foo"));
    assertThat(!isValidDate("200-12-20"));
    assertThat(!isValidDate("2000-0-0"));
    assertThat(!isValidDate("--"));
    return assertThat(!isValidDate(null));
  });

  test("dateToString should return valid date string", function() {
    assertThat(dateToString(new Date(1970, 0, 1)), equalTo("1970-01-01"));
    return assertThat(dateToString(new Date(2011, 11, 12)), equalTo("2011-12-12"));
  });

  test("dateFromString should return valid dates", function() {
    assertThat(dateFromString("2011-12-12"), dateEquals(new Date(2011, 11, 12)));
    return assertThat(dateFromString("1970-01-01"), dateEquals(new Date(1970, 0, 1)));
  });

  test("date chaining conversion should always result to the same value", function() {
    return assertThat(dateFromString(dateToString(dateFromString("2011-12-12"))), dateEquals(dateFromString("2011-12-12")));
  });

  module("month utils tests");

  test("isValidMonth should return true", function() {
    return assertThat(isValidMonth("2011-12"));
  });

  test("isValidMonth should return false", function() {
    assertThat(!isValidMonth(""));
    assertThat(!isValidMonth("foo"));
    assertThat(!isValidMonth("200-12-20"));
    assertThat(!isValidMonth("2000-0"));
    assertThat(!isValidMonth("--"));
    return assertThat(!isValidMonth(null));
  });

  test("monthToString should return valid month string", function() {
    assertThat(monthToString(new Date(1970, 0)), equalTo("1970-01"));
    return assertThat(monthToString(new Date(2011, 11)), equalTo("2011-12"));
  });

  test("monthFromString should return valid dates", function() {
    assertThat(monthFromString("2011-12"), dateEquals(new Date(2011, 11)));
    return assertThat(monthFromString("1970-01"), dateEquals(new Date(1970, 0)));
  });

  test("month chaining conversion should always result to the same value", function() {
    return assertThat(monthFromString(monthToString(monthFromString("2011-12"))), dateEquals(monthFromString("2011-12")));
  });

  module("week utils tests");

  test("isValidWeek should return true", function() {
    assertThat(isValidWeek("2011-W12"));
    return assertThat(isValidWeek("1970-W07"));
  });

  test("isValidWeek should return false", function() {
    assertThat(!isValidWeek(""));
    assertThat(!isValidWeek("foo"));
    assertThat(!isValidWeek("200-W1"));
    assertThat(!isValidWeek("20-W00"));
    assertThat(!isValidWeek("-W"));
    return assertThat(!isValidWeek(null));
  });

  test("weekToString should return valid week string", function() {
    ...
    return assertThat(weekToString(new Date(2010, 4, 11)), equalTo("2010-W19"));
  });

  test("weekFromString should return valid dates", function() {
    ...
    return assertThat(weekFromString("2010-W19"), dateEquals(new Date(2010, 4, 10)));
  });

  test("week chaining conversion should always result to the same value", function() {
    return assertThat(weekFromString(weekToString(weekFromString("2011-W12"))), dateEquals(weekFromString("2011-W12")));
  });

  module("datetime utils tests");

  test("isValidDateTime should return true", function() {
    assertThat(isValidDateTime("2011-16-10T10:45:32"));
    return assertThat(isValidDateTime("2011-16-10T10:45:32.786"));
  });

  test("isValidDateTime should return false", function() {
    assertThat(!isValidDateTime(""));
    assertThat(!isValidDateTime("foo"));
    assertThat(!isValidDateTime("2011-16-10"));
    assertThat(!isValidDateTime("10:15:75"));
    assertThat(!isValidDateTime("2011-16-10T"));
    assertThat(!isValidDateTime("T10:15:75"));
    assertThat(!isValidDateTime("-W"));
    return assertThat(!isValidDateTime(null));
  });

  test("datetimeToString should return valid datetime string", function() {
    ...
    test("datetimeFromString should return valid dates", function() {});
    assertThat(datetimeFromString("2012-03-25T16:44:37"), dateEquals(new Date(2012, 2, 25, 16, 44, 37)));
    return assertThat(datetimeFromString("2012-03-25T16:44:37.756"), dateEquals(new Date(2012, 2, 25, 16, 44, 37, 756)));
  });

  test("datetime chaining conversion should always result to the same value", function() {
    return assertThat(datetimeFromString(datetimeToString(datetimeFromString("2012-03-25T16:44:37"))), dateEquals(datetimeFromString("2012-03-25T16:44:37")));
  });
  ...
  testGenericDateWidgetBehaviors = function(opt) {
    ...
    test("Creating a " + opt.classname + " without argument should setup a default Date", function() {
      ...
      d = new Date;
      input = new opt.cls;
      return assertThat(input.get("date").valueOf(), closeTo(d.valueOf(), 1000));
    });
    ...
    test("" + opt.classname + " should support the min, max and step attributes of the target", function() {
      var input;
      ...
      return assertThat(input.get("step"), strictlyEqualTo(opt.stepValue));
    });
    test("" + opt.classname + " should prevent invalid values to be set in the target", function() {
      ...
      return assertThat(input.get("step"), strictlyEqualTo(0));
    });
    test("" + opt.classname + " should adjust values below the min attribute to fit in the range", function() {
      var input;
      ...
      return assertThat(input.get("date"), dateEquals(opt.minDate));
    });
    test("" + opt.classname + " should adjust dates below the min attribute to fit in the range", function() {
      var input;
      target = $("<input type='" + opt.type + "' value='" + opt.defaultValue + "' min='" + opt.minValue + "' max='" + opt.maxValue + "'></input>")[0];
      input = new opt.cls(target);
    ...
    test("" + opt.classname + " should adjust values above the max attribute to fit in the range", function() {
      var input;
      target = $("<input type='" + opt.type + "' value='" + opt.defaultValue + "' min='" + opt.minValue + "' max='" + opt.maxValue + "'></input>")[0];
      input = new opt.cls(target);
      input.set("value", opt.invalidMaxValue);
      assertThat(input.get("value"), equalTo(opt.maxValue));
      return assertThat(input.get("date"), dateEquals(opt.maxDate));
    });
    test("" + opt.classname + " should adjust dates above the max attribute to fit in the range", function() {
      var input;
      target = $("<input type='" + opt.type + "' value='" + opt.defaultValue + "' min='" + opt.minValue + "' max='" + opt.maxValue + "'></input>")[0];
      input = new opt.cls(target);
    ...
    test("Changing the value should change the target value as well", function() {
      ...
      input.set("value", opt.setValue);
      return assertThat(target.attr("value"), equalTo(opt.setValue));
    });
    return test("Changing the date should change the target value as well", function() {
      var input;
      target = $("<input type='" + opt.type + "' value='" + opt.defaultValue + "'></input>");
      input = new opt.cls(target[0]);
