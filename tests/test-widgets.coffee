$( document ).ready ->
    ...
    test "Changes made to a widget's property should
          dispatch a propertyChanged signal", ->
    ...
    test "Changes made to the target's value should
          be catched by the widget", ->
    ...
    test "Widgets without target should allow modification
          of their properties", ->
    ...
    test "Widgets should allow to modify several properties
          with just one call to the set method", ->
    ...
    test "Widget class should allow subclasses to create
          a dummy in constructor", ->
        ...
        assertThat widget.dummy, allOf notNullValue(),
                                       hasProperty "length", equalTo 1

    test "Widgets that create dummy should have registered
          to its mouse events", ->
    ...
    test "Widgets that create dummy sould allow to unregister
          from its mouse events", ->
    ...
    test "Widget's states should be reflected on the dummy
          class attribute", ->
    ...
    test "Widget's states should have the same order whatever
          the call order is", ->
    ...
    test "Widget should dispatch a stateChanged signal
    ...
    test "Widget shouldn't dispatch the stateChanged
          signal when a property that doesn't affect
    ...
    test "Widget's dummy should allow focus by setting
    ...
    test "Widgets dummy should reflect the focus state
    ...
    test "Widgets dummy should reflect the lost focus
    ...
    test "Widgets should preserve the initial class
    ...
    test "Widgets should allow to register function
          for specific keystrokes on keydown", ->
    ...
    test "Widgets should trigger the corresponding function
          with the associated keystroke on keydown", ->
    ...
    test "Widgets should allow to register function
          for specific keystrokes on keyup", ->
    ...
    test "Widgets should trigger the corresponding function
          with the associated keystroke on keyup", ->
    ...
    test "Widgets keyup events that doesn't trigger a command
    ...
    test "Widgets should provide a way to reset the target
          input to its original state", ->
    ...
    test "Widget's properties getters and setters should
          be overridable in children classes", ->
        setterCalled = false
    ...
    test "Widget's custom properties should be overridable
          in children classes", ->
        setterCalled = false
    ...
    test "Widget's value setter shouldn't dispatch a value
          changed when set is called with the current value", ->
    ...
    test "When both target and dummy exist and target as a style
          attribute, the value should be copied to the dummy", ->
    ...
    test "Widgets should provide a way to know when the widget
    ...
    test "When a widget's target have an id, the widget's dummy
    ...
    test "Widget's should provide a required property mapped
          on the input attribute", ->
    ...
    test "The required state should be reflected on the dummy's
          class attribute", ->
        target = $("<input type='text' required></input>")[0]
        class MockWidget extends Widget
            createDummy:->
    ...
    test "Widget should provide an attach method to place
    ...
    test "Widget should provide a attached signal dispatched
          on a call to attach", ->
    ...
    test "Widget should provide a detached signal dispatched
          on a call to detach", ->
