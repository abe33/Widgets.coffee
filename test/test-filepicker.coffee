$( document ).ready ->
  ...
  test "A FilePicker shouldn't allow a target of with
      a type different than file", ->
  ...
  test "A FilePicker should create a target when
  ...
  test "The FilePicker's value span should contains
      a default value when no file was picked", ->
  ...
  test "A readonly FilePicker should hide its target", ->
    ...
    assertThat picker.dummy.children("input").attr("style"),
  ...
  test "A change made to the target that end with an undefined
      value should empty the dummy's title attribute", ->
